      ...
      subtopics: [
        {
          ...
          body: [
            ...
            "The protocol over $\\mathbb{Z}_p^*$: agree on public parameters - a large prime $p$ and a generator $g$ of $\\mathbb{Z}_p^*$ (or a large prime-order subgroup). Alice generates a secret $a \\leftarrow_R \\{2, \\ldots, p-2\\}$ and sends $A = g^a \\bmod p$. Bob generates a secret $b \\leftarrow_R \\{2, \\ldots, p-2\\}$ and sends $B = g^b \\bmod p$. Alice computes $K = B^a = g^{ab} \\bmod p$. Bob computes $K = A^b = g^{ab} \\bmod p$. Both arrive at the same shared value $g^{ab}$, which an eavesdropper - who sees only $g^a$ and $g^b$ - would need to compute $g^{ab}$ from, which is the **computational Diffie-Hellman (CDH) problem**: given $g^a$ and $g^b$, compute $g^{ab}$. This is believed as hard as the discrete logarithm problem in the chosen group.",
            ...
            "The decisional hardness assumption, **DDH (Decisional Diffie-Hellman)**, is stronger than CDH: given a triple $(g^a, g^b, g^c)$, it is computationally infeasible to determine whether $c = ab \\pmod{p-1}$ or $c$ is random. DDH implies CDH but not vice versa. Security proofs for many DH-based protocols (El Gamal encryption, DH in the random oracle model) rely on DDH. In some groups, DDH is *easier* than DLP - in particular, in bilinear-pairing groups, the Weil and Tate pairings can solve DDH, which is why pairing-based cryptography (BLS signatures, zk-SNARKS) uses specially constructed curves rather than standard DH groups.",
          ],
          resources: [
